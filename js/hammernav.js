...
var HammerNav = function(nav, siblings, toggler, parent, opts ){
    this.ismobile=false;
    var opt_classes = opts.replace(/(\S+)/g, "hammernav-$1");


    this.$nav = $(nav).addClass('hammernav-nav hammernav-item ' + opt_classes);
    this.$siblings = $(siblings).addClass('hammernav-sibling hammernav-item ' + opt_classes);
    this.$items = this.$nav.add(this.$siblings);
    this.$toggler = $(toggler).addClass('hammernav-toggler');
    $(parent).addClass('hammernav-parent');
    ...
    this.$nav.prepend(
        $('<a>').html('X').addClass('hammernav-close').click(function(){ hammernav.close() })
    );
...
HammerNav.prototype.initSubmenus=function(){
    this.$nav.find(" ul li > a ~ ul").prev().addClass("hammernav-subnav-expander-container").append(
        $("<span>")
            .addClass("hammernav-subnav-expander")
            .html("+")
            .click(function(e){
                e.preventDefault();
